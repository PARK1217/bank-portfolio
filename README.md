...
본 전공은 실용음악입니다. 음악 그만두고 자바 백엔드로 일한 지 3년 됐어요. 작년쯤 Home Assistant 로 집 자동화를 해보면서 "기계가 알아서 도는 일" 자체에 흥미가 붙었고, 그 다음에 MCP·LLM 도구를 써보면서 **단순 시간 스케줄이 아니라 상황을 보고 판단해서 움직이는 자동화** 를 만들 수 있겠다 싶었습니다. 그쪽으로 가보고 싶어서 학습 겸 포트폴리오로 만든 게 이 프로젝트입니다.

AI 시작한 지는 두 달 정도밖에 안 됐습니다. 그래서 README 의 ML/LLM 쪽 설명에는 익숙해지려고 일부러 풀어 쓴 표현이 많이 들어가 있습니다.
